...
export async function runAnalysis(runId: ObjectId, url: string, log: Log) {
  ...
  log(`Read ${sources.length} page${sources.length === 1 ? "" : "s"}${removed ? `, skipped ${removed} repeated menu and footer lines` : ""}`);
  ...
  await save({ stage: "profiled", profile, name: profile.name || name });
...
${corpus(sources)}`);
  // ---- D2. Verify: every quote must be in the sources; then judge whether it shows the task or only suggests it
  ...
  await save({ stage: "assessed", assessment: final, verification: report });
  log(`Checked ${report.checked} quotes: ${report.dropped} weren't in the sources and were dropped, ${report.unrelated} didn't support their claim`);
  log(`Found ${final.frictionSignals.length} things that eat time`);
  const assessmentFinal = final;

  // ---- E. Rank  (feeds C4 and Screen 3)
  log("Picking the one tool worth building");
  const answerable = assessmentFinal.coverage.questions.filter(q => q.answerable).length;
...
Friction signals: ${JSON.stringify(assessmentFinal.frictionSignals.map(f => ({ task: f.task, hoursPerWeek: f.hoursPerWeek, confidence: f.confidence, template: f.template })))}
Coverage: ${answerable} of ${assessmentFinal.coverage.questions.length} customer questions are answerable from the site today.
...
present (a support assistant needs answerable questions). belowThreshold = true if the best hoursNow is under 1.`);
  ...
  return { name, profile, benchmark, assessment: assessmentFinal, ranking };
}
