 ...
 * One series hue (blue) for magnitude; ochre only where a second series has to be told apart.
 * Every chart has a table twin so nothing depends on colour or hover alone.
 */

const C = { series: "#2458A0", second: "#8f5d13", grid: "#CAD1CE", axis: "#b3bcb8", ink: "#14213D", muted: "#5F6B72", surface: "#F5F7F6" };
...
export const DEFLECTION: { tool: string; answered: number; handedOff: number }[] = [
  ...
  { tool: "Support & FAQ assistant", answered: 74, handedOff: 26 },
      ...
      {table ? (
        ...
        <div ref={ref} className="relative" style={{ width: "100%" }}>
          ...
          >
            ...
            <text x={x(last) - 8} y={y(HOURS_BY_MONTH[last]) - 10} fontSize={13} fontWeight={600} fill={C.ink} textAnchor="end">
          ...
          {hover !== null && (
            ...
            >
              ...
              <div style={{ color: "#c9d1dc" }}>{LIVE_BY_MONTH[hover]} live assistants</div>
            </div>
          )}
        </div>
      )}
