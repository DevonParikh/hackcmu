  ...
  if (done) {
    return (
      <div className="card p-6 fade-in" role="status">
        ...
        <h2 className="display mt-3 text-2xl font-semibold">Thanks, {name.split(" ")[0] || "we have it"}.</h2>
  ...
  return (
    <form onSubmit={submit} className="card grid gap-4 p-6">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        ...
        <span className="text-sm text-muted">No account needed. We reply within one working day.</span>
      </div>
    </form>
  );
}
